...
fasta=f"{WS}/dca-actin/data/aln/actin_profilin_paired_aug.fasta"
t=time.time()
...
np.save(f"{WS}/handoff/paired_di_aug.npy", arr)
json.dump({"neff":float(mf.effective_num_sequences),"L":L,"npairs":len(out)},
          open(f"{WS}/handoff/paired_di_aug_meta.json","w"))
log(f"SAVED paired_di_aug.npy ({len(out)} pairs)")
